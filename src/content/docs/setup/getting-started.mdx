...
有两种安装方式。"模板"方式轻量简单，但难以更新；而"Fork"方式易于更新，但需要一些 git 技能。

### 使用模板安装

<Aside type='tip'>
Astro 有一篇很好的文章教你设置项目：[安装和设置 Astro #使用主题或入门模板](https://docs.astro.build/en/install-and-setup/#use-a-theme-or-starter-template)
</Aside>
...
1. 安装主题
   ...
   ```
   <Aside type='tip'>
   对于 Bun，如果安装速度较慢，建议通过在项目根目录下创建 `bunfig.toml` 来使用镜像配置：
   ...
   默认情况下，此命令将使用模板仓库的 main 分支。要使用不同的分支名称，请将其作为 `--template` 参数的一部分传递：`cworld1/astro-theme-pure#<branch>`。

2. 回答问题并按照 CLI 向导的说明操作。

3. 完成！

   你现在可以[启动 Astro 开发服务器](https://docs.astro.build/en/install-and-setup/#start-the-astro-dev-server)，在修改项目的同时查看实时预览！

</Steps>

### 使用 Fork 安装

你只需要[点击主题仓库的 fork 按钮](https://github.com/cworld1/astro-theme-pure/fork)来创建你的项目；然后将 fork 的仓库克隆到本地。

```shell
git clone https://github.com/<your-username>/astro-theme-pure.git
```

然后，你可以启动 Astro 开发服务器，在修改项目的同时查看实时预览！
...
参见 [Astro：将现有项目迁移到 Astro](https://docs.astro.build/en/guides/migrate-to-astro/)。
...
1. 删除文档文件
   ...
   - 删除 `src/content.config.ts` 中文档的内容集合：
...
3. 更改站点图标。
   ...
   <Aside type='tip'>
   [Favicon.io](https://favicon.io/) 是一个很好的工具，可以帮助你快速生成图标。
   </Aside>

   用你自己的图标替换 `public/favicon/*` 文件。
...
6. TypeScript 语法
   ...
   配置文件 `site.config.ts` 使用 TypeScript 语法。如果你不熟悉 TS 语法，请先阅读[这里](https://learnxinyminutes.com/docs/typescript/)。
