---
title: '高级组件'
description: '高级组件及其文档'
order: 2
---

import { MdxRepl } from 'astro-pure/user'

## 网页内容渲染

### 随机引言

`Quote` 组件是一个简单的组件，用于显示随机引言。

<MdxRepl>
import { Quote } from 'astro-pure/advanced'

<Quote />

<Fragment slot='desc'>
```jsx
import { Quote } from 'astro-pure/advanced'

<Quote />
```
</Fragment>
</MdxRepl>

配置：

```ts title="src/site.config.ts"
export const integ: IntegrationUserConfig = {
  // 在页脚添加随机引言（默认在主页页脚）
  quote: {
    // https://github.com/lukePeavey/quotable
    server: 'https://api.quotable.io/quotes/random?maxLength=60',
    target: `(data) => data[0].content || 'Error'`
  }
}
```

[Quotable](https://github.com/lukePeavey/quotable) 是一个提供随机英文引言的开源 API。

对于中文引言，你可以使用[一言](https://developer.hitokoto.cn/sentence/)。

```ts title="src/site.config.ts"
export const integ: IntegrationUserConfig = {
  quote: {
    // https://developer.hitokoto.cn/sentence/
    server: 'https://v1.hitokoto.cn/?c=i',
    target: `(data) => data.hitokoto || 'Error'`
  }
}
```

### GitHub 卡片

`GithubCard` 组件是一个简单的组件，用于显示 GitHub 用户卡片。

<MdxRepl width='80%'>
import { GithubCard } from 'astro-pure/advanced'

<GithubCard repo='cworld1/astro-theme-pure' />

<Fragment slot='desc'>
```jsx
import { GithubCard } from 'astro-pure/advanced'

<GithubCard repo='cworld1/astro-theme-pure' />
```
</Fragment>
</MdxRepl>

### 链接预览

预览内容中插入的任何链接。

<MdxRepl width='80%'>
import { LinkPreview } from 'astro-pure/advanced'

<LinkPreview href='https://www.mozilla.org/en-US/' />
<LinkPreview href='https://www.cloudflare.com/' hideMedia />

<Fragment slot='desc'>
```jsx
import { LinkPreview } from 'astro-pure/advanced'

<LinkPreview href='https://www.cloudflare.com/' />
// `zoomable` 选项将控制 mediumzoom 标签
// <LinkPreview href='https://www.cloudflare.com/' zoomable={false} />
<LinkPreview href='https://www.cloudflare.com/' hideMedia />
```
</Fragment>
</MdxRepl>

## 数据转换器

### 二维码

<MdxRepl>
import { QRCode } from 'astro-pure/advanced'

<QRCode
  content='https://github.com/cworld1/astro-theme-pure'
  class='inline-flex max-w-44 p-3 bg-muted rounded-lg border' />

<Fragment slot='desc'>
```jsx
import { QRCode } from 'astro-pure/advanced'

// <QRCode /> // 默认渲染当前页面链接
<QRCode
  content='https://github.com/cworld1/astro-theme-pure'
  class='inline-flex max-w-44 p-3 bg-muted rounded-lg border' />
```
</Fragment>
</MdxRepl>

### 提示消息

<MdxRepl>
import { Button } from 'astro-pure/user'

<Button
  title='点击我'
  onClick={`document.dispatchEvent(
    new CustomEvent('toast', {
      detail: {
        message: '来自提示消息的问候！',
        time: 5000
      }
    })
  )`} />
<Fragment slot='desc'>
```jsx
import { Button } from 'astro-pure/basic'

<Button
  title='点击我'
  onClick={`document.dispatchEvent(
    new CustomEvent('toast', {
      detail: {
        message: '来自提示消息的问候！',
        time: 5000
      }
    })
  )`} />
```
</Fragment>
</MdxRepl>

### 图片缩放

尝试点击下面的图片。

![img](https://github.com/user-attachments/assets/6c42b061-df7e-4696-a29b-bff07fe17d88)

参见[其他集成#medium-zoom](/docs/integrations/others#medium-zoom)。
