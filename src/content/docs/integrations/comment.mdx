...
<Aside>
  本主题已集成 Waline 评论系统。你可以在 `src/site.config.ts` 文件的 `integrationConfig.waline` 中配置它。
</Aside>
...
如果你有理由移除评论系统，或只是想了解如何更换其他评论系统，本节可能对你有帮助。但如果你想了解如何配置它，可以跳过本节。
...
5. 在以下文件中移除之前编辑的布局的相关配置（如 `comment` 或 `view`）：
